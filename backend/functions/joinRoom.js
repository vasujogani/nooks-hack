...
exports.joinRoom = functions.https.onRequest((request, response) => {
  ...
  if (
    request.body.link_id == null ||
    request.body.link_id === "" ||
    typeof request.body.link_id === "undefined"
  ) {
  ...
  linkRef.get().then((snapshot) => {
    ...
    if (
      snapshot.data().session_id == null ||
      snapshot.data().session_id === "" ||
      typeof snapshot.data().session_id === "undefined"
    ) {
